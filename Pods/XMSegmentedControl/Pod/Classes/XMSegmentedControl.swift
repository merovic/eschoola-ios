...
public class XMSegmentedControl: UIView {
        ...
        didSet {
            ...
            UIView.animate(withDuration: 0.3, delay: 0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.7, options: UIView.AnimationOptions.curveEaseOut, animations: {
                ...
                case .Text:
                    ((self.subviews.filter(isUIButton)) as! [UIButton]).forEach {
                        if $0.tag == self.selectedSegment {
                            $0.setTitleColor(self.highlightTint, for: [])
                            self.highlightView.frame.origin.x = $0.frame.origin.x
                        } else {
                            $0.setTitleColor(self.tint, for: [])
                        }
                    }
                }
    ...
    public init (frame: CGRect, segmentTitle: [String], selectedItemHighlightStyle: XMSelectedItemHighlightStyle) {
        ...
        self.commonInit(data: segmentTitle, highlightStyle: selectedItemHighlightStyle)
    }
    ...
    public init (frame: CGRect, segmentIcon: [UIImage], selectedItemHighlightStyle: XMSelectedItemHighlightStyle) {
        ...
        self.commonInit(data: segmentIcon, highlightStyle: selectedItemHighlightStyle)
    }
    ...
    public init (frame: CGRect, segmentContent: ([String], [UIImage]), selectedItemHighlightStyle: XMSelectedItemHighlightStyle) {
        ...
        self.commonInit(data: segmentContent, highlightStyle: selectedItemHighlightStyle)
    }
    ...
    public convenience init (frame: CGRect, verticalSegmentContent: ([String], [UIImage]), selectedItemHighlightStyle:XMSelectedItemHighlightStyle) {
        ...
        setupVerticalSegmentContent(content: verticalSegmentContent)
    }
    ...
    public func update() {
        func addSegments(startingPosition starting: CGFloat, sections: Int, width: CGFloat, height: CGFloat) {
            for i in 0..<sections {
                ...
                case .Icon:
                    ...
                    tab.setImage(segmentIcon[i], for: [])
                case .Text:
                    tab.setTitle(segmentTitle[i], for: [])
                    tab.setTitleColor(i == selectedSegment ? highlightTint : tint, for: [])
                ...
                case .Hybrid:
                    ...
                    tab.setTitle(segmentContent.text[i], for: [])
                    tab.setImage(segmentContent.icon[i], for: [])
                    tab.titleLabel?.font = font
                ...
                case .HybridVertical:
                    ...
                    tab.imageEdgeInsets = UIEdgeInsets(top: insetAmount*2, left: horizontalInset, bottom: height - imageSize.height + insetAmount, right: horizontalInset)
                    tab.titleEdgeInsets = UIEdgeInsets(top: height - bottomTitleInset, left: -imageSize.width / 2, bottom: insetAmount*2, right: imageSize.width / 2)
                    tab.contentEdgeInsets = UIEdgeInsets(top: 0, left: insetAmount, bottom: 0, right: insetAmount)
                    tab.contentHorizontalAlignment = .center
                    tab.setTitle(segmentContent.text[i], for: [])
                    tab.setImage(image, for: [])
                ...
                tab.tag = i
                tab.addTarget(self, action: Selector(("segmentPressed:")), for: .touchUpInside)
                self.addSubview(tab)
            }
        }
        ...
        if contentType == .Text {
            ...
            let positionWidth = startingPositionAndWidth(totalWidth: totalWidth, distribution: itemWidthDistribution, segmentCount: tabBarSections, selectedIndex: selectedSegment)
    ...
    public func pressTabWithIndex(index: Int) {
        for subview in self.subviews where subview.tag == index {
            if subview is UIButton {
                segmentPressed(sender: subview as! UIButton)
                return
            }
        }
    }
